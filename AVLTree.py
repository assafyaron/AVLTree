        ...
        ###################################
                ...
                if balanceFactor == -2: # balance right subtree
                        ...
                        #           n2      =>                       
                                ...
                                raise Exception("Tried to deal with unbalanced right child " + str(n1.get_key()) + "with incorrect Bf: " + str(n1BalanceFactor))
	...
	"""
        def split(self, node):
                ...
                if node.get_left().is_virtual():
                        ...
                        leftTree.root.set_parent(None)

                if node.get_right().is_virtual():
                        rightTree.root = None
                else:
                        rightTree.root = node.get_right()
                        rightTree.root.set_parent(None)
                ...
                while currParent != None:
                        ...
                        if currChild.is_inner_node_right_son():
                                ...
                                leftTree.join(tempTree, currParent.get_key(),currParent.get_value())
                                ...
                                currParent.set_height(0)

                                # join the current subtrees into one using helper function
                                # notice actual_join returns integer which doesn't concern us during split
                                rightTree.join(tempTree, currParent.get_key(),currParent.get_value())
	...
	"""
        def actual_join(self, tree2, joinNode):
                ...
                # 2nd case: big_key_tree is larger than the small_key_tree
                        ...
                        temp.set_height(max(joinNode.get_height(),temp.get_right().get_height())+1)
                ...
                # 3rd case: small_key_tree is larger than the big_key_tree
                        ...
                        temp.set_height(max(joinNode.get_height(),temp.get_left().get_height())+1)
